...
已完成 Day 01–15（linked list → Kruskal）。以下為 Day 16–100 的規劃。
...
- 強化學習從 10 天擴成 **20 天**，後 10 天直接對著 [radixark/miles](https://github.com/radixark/miles)
  （SGLang rollout + Megatron trainer 的企業級 LLM/VLM post-training 框架，從 slime fork）
  的功能清單走：GRPO/GSPO、TITO、weight sync、fully async、MoE routing replay、
  低精度訓練、agentic 環境與容錯。學會的是「RL 訓練框架長什麼樣」，不只是演算法。
...
| Day | 主題 | 備註 |
|---|---|---|
| 20 | 分治排序：Quick sort (與 partition | 三路劃分、最壞情況) + Merge sort(與外部排序 | 大檔案排序) + 它們的平行版 ；parallel merge、sample sort、work-depth 直覺 |
...
## Part 10 — 強化學習基礎（Day 72–81）
從 bandit 一路推到 PPO 與 off-policy 修正，這十天是後面十天的前提。

| Day | 主題 | 備註 |
|---|---|---|
| 72 | Multi-Armed Bandit：ε-greedy、UCB、Thompson | 接 Day 42 隨機化；對照 Day 60 取樣溫度 |
| 73 | MDP 與 Value Iteration | GridWorld |
| 74 | Policy Iteration 與 RL 裡的 DP | 接 Part 5 |
| 75 | Monte Carlo 與 TD Learning | bias-variance 的第一次現身 |
| 76 | Q-Learning（表格式） | on-policy vs off-policy 的分水嶺 |
| 77 | DQN：replay buffer、target network | replay buffer 就是 Day 21 的取樣結構 |
| 78 | REINFORCE 與 policy gradient | log-prob 技巧、baseline 降變異 |
| 79 | Actor-Critic / A2C / GAE | |
| 80 | PPO：ratio clipping、KL 懲罰、優勢正規化 | Day 84 GRPO 的對照組 |
| 81 | Off-policy 修正與重要性取樣 | behaviour vs target policy；Day 88 async RL 的理論基礎 |

## Part 11 — LLM 後訓練與 RL 基礎設施（Day 82–91）
對照 [Miles](https://github.com/radixark/miles) / slime / SGLang 的實作，把「RL 訓練框架長什麼樣」拆開來寫。
每天都是可跑的小型復刻，不是讀 code 心得。

| Day | 主題 | 備註 |
|---|---|---|
| 82 | RLHF 全景與 reward model 從零訓 | 偏好資料 → Bradley-Terry loss → RM |
| 83 | DPO / SimPO：不要 reward model 的對齊 | 把 Day 56 的 mini-GPT 拿來 align |
| 84 | GRPO / GSPO / REINFORCE++：丟掉 critic 的 LLM RL | group-relative advantage；Miles 的 recipe 清單 |
| 85 | RLVR：可驗證獎勵、數學與程式的 verifier | reward hacking 與 reward shaping 的實例 |
| 86 | Rollout engine：把推論引擎當環境 | 接 Day 65；token-in-token-out (TITO)、為什麼不要 detokenize 再 retokenize |
| 87 | Trainer ↔ rollout 的權重同步 | weight transfer、更新頻率、on-policy 落後多少還算 on-policy |
| 88 | Fully async RL：解耦 rollout 與 training worker | 接 Day 43 asyncio + Day 81 off-policy 修正；staleness 排程 |
| 89 | 分散式訓練切法與 MoE routing replay (R3) | DP/TP/PP/EP 的取捨；接 Day 63，路由不一致為什麼會炸掉大 run |
| 90 | 低精度 RL 訓練與數值穩定 | 接 Day 62；FP8 / MXFP8 / NVFP4、precision-induced divergence、KL 與 entropy 當監控訊號 |
| 91 | Agentic RL：multi-turn rollout、工具環境、沙箱與容錯 | 環境連接器、engine 掛掉怎麼原地復原；銜接 Part 13 專案 |

## Part 12 — 搜尋與演化計算（Day 92–94）

| Day | 主題 | 備註 |
|---|---|---|
| 92 | MCTS（AlphaZero-lite，井字棋 / 連四） | 接 Day 39 回溯 + Day 19 A* + Day 72 UCB |
| 93 | 基因演算法：編碼、選擇、交配、突變 | 用 GA 解 Day 37 的 TSP |
| 94 | 無梯度優化橫向比較：模擬退火、PSO、爬山、ES / CMA-ES | 演化 vs 梯度，對照 Day 78 policy gradient |
...
| 99 | Eval Harness：LLM-as-judge、回歸測試、追蹤 | 也是 Day 85 verifier 的延伸 |
...
- Day 22 prefix-sum (scan) → Day 43 平行模型 → Day 88 async 排程
...
- Day 37 狀壓 TSP → Day 93 GA 解 TSP（精確 vs 啟發式對照）
- Day 38 Huffman → Day 52 BPE
- Day 39 回溯 → Day 92 MCTS
- Day 42 隨機化 → Day 72 bandit → Day 60 取樣策略（探索與溫度是同一件事）
- Day 43 asyncio / work-depth → Day 65 continuous batching → Day 88 fully async RL → Day 95 agent loop
- Day 56 mini-GPT → Day 83 DPO → Day 84 GRPO → Day 95–100 專案
- Day 62 量化 → Day 90 低精度 RL 訓練
- Day 63 MoE 路由 → Day 89 rollout routing replay (R3)
- Day 65 推論排程器 → Day 86 rollout engine（同一個引擎，一個對外服務、一個當 RL 環境）
- Day 76 off-policy / Day 81 重要性取樣 → Day 88 async RL 的 staleness 容忍度
- Day 85 verifier → Day 99 eval harness
