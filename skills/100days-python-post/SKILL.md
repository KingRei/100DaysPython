...
day NN - <topic>/
    ...
    medium_en.md       # Medium article draft, English
    medium_zh.md       # Medium article draft, 繁體中文
    imgs/
        dayNN_1.png
...
Two files, same content and structure, not machine-translated stubs:

- `medium_en.md` - English, first person, teaching tone, 800-1400 words.
- `medium_zh.md` - 繁體中文，同樣的架構與程式碼區塊；技術名詞保留英文（BFS、queue、
  adjacency list…），行文用台灣習慣的說法，不要翻譯得太生硬。程式碼註解維持英文，
  說明文字用中文。
...
Hand back with computer:// links to the notebook, the runnable `.py`, README, both
Medium drafts and the figures, and flag anything that still needs Eric's input
(published Medium URL, ROADMAP tweaks).
